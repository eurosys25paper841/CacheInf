...
class TorchOPProfile:
    def __init__(self, *, idx: int, func_name: str,
        ...
        self.offset = 0
        self.stride = 1
        self.temp_field_len = 0  # receptive field length
        self.field_len = 0  # receptive field length
    ...
    def copy_for_transmission(self):
        ...
        ret.field_len = self.field_len
        ret.caching = self.caching
        ret.local_dim = self.local_dim
        if "profile" in ret.hook_kwargs:
            del ret.hook_kwargs["profile"]
        return ret

    def __repr__(self) -> str:
        return f"{self.idx} {self.func_name}: input_from: {self.input_from} output_to: {self.output_to} output_shapes: {self.output_shapes} barrier: {self.hook_kwargs['barrier']} kernel size {self.kernel_size} local stride {self.local_stride} padding {self.padding} offset {self.offset} stride {self.stride} field_len {self.field_len}; "
