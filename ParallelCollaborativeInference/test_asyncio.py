...
class SockLog:
    ...
    def record_send(self, send_record):
        if len(send_record):
            ...
            self.first_send = send_record[0, 2]
            self.last_send = send_record[-1, 2]
    ...
    def record_recv(self, recv_record):
        if len(recv_record):
            ...
            self.first_recv = recv_record[0, 2]
            self.last_recv = recv_record[-1, 2]
    ...
    def compute_info(self):
        ...
        self.msg = self.prefix + f"send took {send_took:.2e}s ({send_size:.4f}MB, {self.send_num} items), recv took {recv_took:.2e}s ({recv_size:.4f}MB, ({self.recv_num} items)), dumps took {self.dumps_took:.2e}s, loads took {self.loads_took:.2e}s total {sum([send_took, recv_took, self.dumps_took, self.loads_took]):.4f}s bandwidth {bw:.4f}MB/s, first send at {self.first_send}, last send at {self.last_send}, first recv at {self.first_recv}, last recv at {self.last_recv}" + self.suffix
...
class AsyncTCPMessageStream:
    ...
    def __init__(self, reader_writer: Tuple[StreamReader, StreamWriter],
        ...
        self.header_fmt = 'Ld?' # [long, double, bool]: [msize, send took, ack]
        ...
        self.send_record = []
        self.recv_record = []
        self.temp_send_record = deque(maxlen=3) # Small record size to keep records fresh
        self.temp_recv_record = deque(maxlen=3)
    ...
    async def _send_msg(self, msg):
        ...
        await self.sent_queue.put(None)
        if len(msg) > bw_count_size:
            await self.ctrl_reader.read(1)
        return time.time() - stime

    async def send_msg(self, msg):
        msg = struct.pack(self.header_fmt, len(msg), self.last_send_took, True) + msg
        send_dur = await self._send_msg(msg)
        size = len(msg)
        if size > bw_count_size:
            self.temp_send_record.append([size, send_dur])
        self.send_record.append([size, send_dur, time.time()])
        self.last_send_took = send_dur

    async def pure_send(self, obj):
        msg = pickle.dumps(obj)
        msg = struct.pack(self.header_fmt, len(msg), self.last_send_took, False) + msg
        self.writer.write(msg)
    ...
    async def _recv_msg(self):
        ...
        msize, last_recv_took, ack = struct.unpack(self.header_fmt, header)
        if msize < max_recv_size:
            ...
            while len(buffer) < msize:
                ...
                if not _buffer:
                    raise EOFError
                buffer += _buffer
        if msize + self.header_size > bw_count_size and ack:
    ...
    async def recv_msg(self):
        ...
        msg_size = len(msg) + self.header_size
        if self.last_recv_size: # skip first recv took
            if self.last_recv_size > bw_count_size:
                self.temp_recv_record.append([
                    self.last_recv_size, last_recv_took])
            self.recv_record.append([self.last_recv_size, last_recv_took, self.last_recv_time])
        self.last_recv_time = time.time()
        self.last_recv_size = msg_size
        return msg
    ...
    def fix_init_sock_log(self, prefix="", role="client"):
        self.recording_log.prefix = prefix
        if role == "client" or True:
    ...
    def skip_next_recv(self):
        # This transaction only preamble will be transmitted
        self.skip_recv += 1
