...
# apps = ["agrnav", "kapao"]
apps = ["MobileNet_V3_Small", "ResNet101", "VGG19_BN",
]
...
def parse_local_log(path):
    ...
    with open(latency_file, "r", encoding="utf8") as f:
        ...
        for line in f.readlines():
            if _inference_start_time := find_value(line, r"inference starts at (.*?);"):
    ...
    return inference_time[3:], FPS_time[3:], vdd_in[3:]
    ...
    with open(latency_file, "r", encoding="utf8") as f:
        ...
        for line in f.readlines():
            if _ops_num := find_value(line, r"; total (.*?) ops", int):
                ops_num = _ops_num
            elif _param_num := find_value(line, r"Model parameter number (.*?)M."):
                model_param_num = _param_num
            elif _input_size := find_value(line, r"Input size (.*?)MB"):
                input_size = _input_size
            elif _output_size := find_value(line, r"Output size (.*?)MB"):
                output_size = _output_size
            elif (_send_op := find_value(line, r"op (.*?) [a-z_A-Z0-9]* send", int)) is not None:
                _send_ops.append(_send_op)
            elif _inference_start_time := find_value(line, r"inference starts at (.*?);"):
                if _last_inference_start_time:
                    _fps_time = _inference_start_time - _last_inference_start_time
                else:
                    _fps_time = 0.
                ...
                _send_ops = []
            elif "send took" in line and "Init" not in line:
                if len(send_ops[len(transmission_time)]) > 0:
...
def analyse_inference_info(_results: Dict[str, Dict[str, Dict[str, ItemInfo]]]):
    index = []
    columns = [tuple(zip(["Transmission time/s", "Inference time(s)", "Percentage(\%)"], [mode_to_baseline[mode]]*3)) for mode in modes]
        ...
        for env, val2 in val1.items():
            ...
            for mode, iteminfo in val2.items():
                try:
                    ...
                    _data.append([mean_with_std(trans), mean_with_std(inf),
                                f"{mean_trans/mean_inf*100:.2f}"])
                except Exception:
                    print(f"{app} {env} {mode} has error.")
            # if "RegNet" in app:
            #     _data = [_data[1], _data[0]]
            data.append(np.moveaxis(np.array(_data), 1, 0).ravel().tolist())
            index.append([f"{app}({int(iteminfo.param_num)}M)",
                          mean_with_std(iteminfo.local_comp_time), env])
    data = np.array(data) # [app*env*mode, dim]
    index = pd.MultiIndex.from_tuples(index, names=["Model(number of parameters)", "Local Inference time(s)", "Environment"])
    columns = pd.MultiIndex.from_tuples(columns)
    df = pd.DataFrame(data, index=index,columns=columns)
    column_format = "ccc" + (len(columns) - 1) * "|c" + "|c"
    with open("tp_transmission_percentage.latex", "w") as f:
...
def analyse_power_info(_results: Dict[str, Dict[str, Dict[str, ItemInfo]]]):
    index = []
    columns = [tuple(zip(["Power consumption(W)", "Power consumption per inference(W)"],
                          [mode_to_baseline[mode]]*2)) for mode in modes]
    columns = np.moveaxis(np.array(columns), 1, 0).reshape(-1, 2).tolist()
    data = []
    for app, val1 in _results.items():
        app = "\_".join(app.split("_"))
        for env, val2 in val1.items():
            ...
            for mode, iteminfo in val2.items():
                try:
                    ...
                    mean_pw = np.mean(pw)
                    pw_per_inf = np.array(inf) * mean_pw
                    _data.append([mean_with_std(pw, 2), mean_with_std(pw_per_inf, 2)])
                except Exception:
                    print(f"{app} {env} {mode} has error.")
            # if "RegNet" in app:
            #     _data = [_data[1], _data[0]]
            ...
            data.append(np.moveaxis(np.array(_data), 1, 0).ravel().tolist())
            index.append([f"{app}({int(iteminfo.param_num)}M)",
                        mean_with_std(np.array(iteminfo.local_comp_power_consumption)/1000., 2), mean_with_std(local_comp_time*local_pw/1000., 2), env])
    data = np.array(data) # [app*env*mode, dim]
    index = pd.MultiIndex.from_tuples(index,
                                       names=["Model(number of parameters)", "Local power consumption(W)", "Local power consumption(W) per inference", "Environment"])
    columns = pd.MultiIndex.from_tuples(columns)
    df = pd.DataFrame(data, index=index,columns=columns)
    column_format = "cccc" + (len(columns) - 1) * "|c" + "|c"
    with open("tp_power.latex", "w") as f:
        df.to_latex(f, multicolumn=True, multirow=True, float_format="%.3f",
                     column_format=column_format, multicolumn_format="|c|")
