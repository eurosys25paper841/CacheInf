...
class SemanticKITTI_dataloader(Dataset):
  ...
  def __init__(self, dataset, phase):
    ...
    '''
    origin_file_path = osp.join(osp.abspath(osp.join(*([osp.abspath(__file__)] + [os.pardir] * 5))), "third_parties/AGRNav/src/perception/SCONet/network/data/SemanticKITTI.py")

    yaml_path, _ = os.path.split(os.path.realpath(origin_file_path))
    ...
    self.frame_idx = 0
    self.sequence_len = len(self.filepaths['3D_OCCUPANCY'])
  ...
  def get_filepaths(self, modality):
      ...
      if modality == '3D_OCCUPANCY':
          ...
          src_path = osp.join(osp.abspath(osp.join(*([osp.abspath(__file__)] + [os.pardir] * 5))), "third_parties/AGRNav/src")
          dataset_dir = osp.join(src_path, "perception/raw_data/voxels")
          file_pattern = '*.bin'
          ...
          self.filepaths['3D_OCCUPANCY'] = filepaths
  ...
  def __getitem__(self, idx):
    forward_backward = (self.frame_idx // self.sequence_len) % 2
    if forward_backward == 0:   # forward
        true_idx = self.frame_idx % self.sequence_len
    else:                       # backward
        true_idx = - (self.frame_idx % self.sequence_len + 1)
    self.frame_idx += 1
    idx = true_idx
    ...
    return data, idx, os.path.basename(self.filepaths[modality][idx])
