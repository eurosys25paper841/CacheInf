...
def test(model, dset, _cfg, logger, out_path_root, coordinates_publisher):
    ...
    log_dir = os.environ.get("ROS_LOG_DIR", os.environ["work"])

    start_time = time.time()
    inference_time = []
   
    with torch.no_grad():
        for t, (data, indices, file_name) in enumerate(dset):
            data = dict_to(data, device, torch.uint8)

            # Record the inference start time

            data['3D_OCCUPANCY']  = data['3D_OCCUPANCY'].squeeze(1).permute(0, 2, 1, 3).contiguous()
            ...
            for score in scores['pred_semantic_1_1']:
                score = score.permute(1,0,2)  # [bs, C, H, W, D] -> [bs, C, W, H, D]
                # voxel occupancy file
                input_filename = dset.dataset.filepaths['3D_OCCUPANCY'][indices[curr_index]]
                print(input_filename)

                voxel_occupancy = torch.moveaxis(data['3D_OCCUPANCY'][0], 2, 1).ravel().reshape(256, 32, -1)
                ...
                non_intersection_coordinates = torch.nonzero(non_intersection.reshape(256, 32, 256))
                non_intersection_score = score[torch.nonzero(non_intersection.reshape(256, 32, 256), as_tuple=True)]
                # Save inference output to file
                with open(os.path.join(log_dir, f"infer_{t}.pkl"), "wb") as f:
                    pickle.dump([file_name, non_intersection_coordinates], f)
