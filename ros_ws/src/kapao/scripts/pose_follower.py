...
class CompNavigation():
    ...
    def run_nms(self):
        ...
        model.model[-1].nl_start = 2
        model.model[-1].nl_end = 3
        model.eval()
        ...
        dataset = VideoFrameDataset(os.path.join(os.environ["work"], "data/DAVIS/JPEGImages/1080p"),
                                "tennis")
        if offload:
            ...
            pci = ParallelCollaborativeInference(offload, offload_method, ip, port,
                                                    ...
                                                    log=rospy.loginfo)
            pci.start_client(model, init_forward_count=1, data_range=255., block_size=16, min_stride=128)
        ...
        while not rospy.is_shutdown():
            ...
            with open(os.path.join(log_dir, f"infer_{count}.pkl"), "wb") as f:
                pickle.dump([idx, (person_dets, kp_dets)], f)
            ...
            rospy.loginfo(f"{count} th input; finishes at {time.time()-stime:.4f} s")
            count += 1
