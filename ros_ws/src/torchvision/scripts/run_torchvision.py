...
if __name__ == "__main__":
    ...
    PCI = ParallelCollaborativeInference(offload=offload,
                                            parallel_approach=offload_method,
                                            ip=ip, port=port,
                                            constraint_latency=offload_mode=="fix",
                                            log=rospy.loginfo)
    PCI.start_client(model=model, init_forward_count=1, data_range=4, min_stride=32, block_size=8)
    _dataset = VideoFrameDataset(os.path.join(os.environ["work"], "data/DAVIS/JPEGImages/480p"),
                                "goat")
    ...
    for i, (idx, inp) in enumerate(_dataset):
        if rospy.is_shutdown():
            break
        pred = model(preprocess(Image.fromarray(inp)).to(device).unsqueeze(0).contiguous())
        ...
        with open(os.path.join(log_dir, f"infer_{i}.pkl"), "wb") as f:
            pickle.dump([idx, torch.argmax(pred)], f)
