...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl_start, self.nl_end):
            x[i] = self.m[i](x[i])  # conv
            bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
            # x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            x[i] = x[i].unflatten(1, (self.na, self.no)).permute(0, 1, 3, 4, 2).contiguous()
            ...
            if not self.training:  # inference
                ...
                z_last_temp = z[-1].view(*(x[i].shape))
                z_last_temp[..., 0:2] = (z_last_temp[..., 0:2] + self.grid[i])* self.stride[i]
                if hasattr(self, 'num_coords') and self.num_coords and self.inplace:
                    z_last_temp[..., -self.num_coords:] += (self.grid[i] * self.stride[i]).repeat((1, 1, 1, 1, self.num_coords // 2))
                z[-1] = z_last_temp.view(bs, -1, self.no)
...
class Model(nn.Module):
    ...
    def forward(self, x, data, augment=False, profile=False, visualize=False, kp_flip=None,
                scales=[0.5, 1, 2], flips=[None, 3, None]):
        x = x / 255.0  # 0 - 255 to 0.0 - 1.0
        out = self._forward(x, augment, profile, visualize, kp_flip, scales, flips)[0]
        return run_nms(data, out)

    def _forward(self, x, augment=False, profile=False, visualize=False, kp_flip=None,
                scales=[0.5, 1, 2], flips=[None, 3, None]):
        if augment:
